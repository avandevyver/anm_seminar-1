...
int main()
{
	...
	double kh = compute_kh(nPoints, RADIUS_ALGORITHM) * (xmax-xmin);
	double L = 0.0;
	...
	while (!bov_window_should_close(window)) {
		if (SEARCH_NEIGHBORHOOD) {
			...
			for (int i = 0; i < NTH; i++) {
				...
				loop->finished_threads = finished_threads;
				if(use_verlet)
			...
			//TODO erase should close ?
			while (!bov_window_should_close(window) && protected_get(finished_threads) != NTH) {
